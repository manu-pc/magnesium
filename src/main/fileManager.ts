...
import {
  IPC, AppConfig, DEFAULT_CONFIG, DEFAULT_PDF_EXPORT, DEFAULT_STYLES, DEFAULT_TABLE_STYLE,
  StyleManifest, MarkdownStyles, CustomFormatRule, TableStyle
} from '../shared/types'
...
// Table styles for each built-in preset
const defaultTable: TableStyle = DEFAULT_TABLE_STYLE
...
const BUILTIN_PRESETS: Record<string, BuiltinPreset> = {
  'default.json': {
    ...
    impliedTheme: 'light',
    customRules: [],
    markdownStyles: { ...DEFAULT_STYLES, table: defaultTable }
  },
  'github.json': {
    version: 3,
    name: 'GitHub',
...
function migrateConfig(config: Record<string, any>, store: Store<AppConfig>): Record<string, any> {
  ...
  // Migration 5: table style default
  if (config.markdownStyles && !config.markdownStyles.table) {
    config.markdownStyles.table = DEFAULT_TABLE_STYLE
    store.set('markdownStyles', config.markdownStyles)
  }
...
export function registerFileHandlers(win: BrowserWindow, store: Store<AppConfig>): void {
  ...
  ipcMain.handle(IPC.SHOW_OPEN_DIALOG, async () => {
    const result = await dialog.showOpenDialog(win, {
      filters: [
        { name: 'All Supported', extensions: ['md', 'markdown', 'txt', 'c', 'h', 'cpp', 'cc', 'cxx', 'hpp', 'java', 'py', 'html', 'htm', 'css', 'js', 'ts'] },
        { name: 'Markdown', extensions: ['md', 'markdown', 'txt'] },
        { name: 'C / C++', extensions: ['c', 'h', 'cpp', 'cc', 'cxx', 'hpp'] },
        { name: 'Java', extensions: ['java'] },
        { name: 'Python', extensions: ['py'] },
        { name: 'Web', extensions: ['html', 'htm', 'css', 'js', 'ts'] },
      ],
      properties: ['openFile']
    })
  ...
  ipcMain.handle(IPC.STYLE_IMPORT, async () => {
    ...
    try {
      ...
      const importedStyles = parsed.markdownStyles
      if (!importedStyles.table) {
        importedStyles.table = DEFAULT_TABLE_STYLE
      }
      return {
        success: true,
        markdownStyles: importedStyles,
