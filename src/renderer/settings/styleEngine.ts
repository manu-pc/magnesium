import type { MarkdownStyles, ElementStyle, CustomFormatRule, TableStyle, SyntaxColors } from '../../shared/types'
import { DEFAULT_TABLE_STYLE } from '../../shared/types'
...
export function generateTableCSS(style: TableStyle, forPrint = false): string {
  const parts: string[] = []

  parts.push(`#preview-content .table-wrapper {
  width: ${style.width};
  overflow-x: ${forPrint ? 'visible' : 'auto'};
  border-radius: ${style.borderRadius}px;
  border: ${style.outerBorder};
  margin: 1em 0;
}`)

  parts.push(`#preview-content table {
  width: 100%;
  border-collapse: ${style.borderCollapse};
  border-spacing: 0;
}`)

  parts.push(`#preview-content thead {
  background-color: ${style.headerBackground};
}`)

  parts.push(`#preview-content th {
  font-family: ${style.header.fontFamily};
  font-size: ${style.header.fontSize}px;
  font-weight: ${style.header.fontWeight};
  font-style: ${style.header.fontStyle};
  color: ${style.header.color};
  background-color: ${style.header.backgroundColor};
  text-align: ${style.header.textAlign};
  padding: ${style.header.padding}px;
  border-bottom: ${style.headerBorder};
  border-right: ${style.cellBorder};
}`)

  parts.push(`#preview-content th:last-child { border-right: none; }`)

  parts.push(`#preview-content td {
  font-family: ${style.cell.fontFamily};
  font-size: ${style.cell.fontSize}px;
  font-weight: ${style.cell.fontWeight};
  font-style: ${style.cell.fontStyle};
  color: ${style.cell.color};
  background-color: ${style.cell.backgroundColor};
  text-align: ${style.cell.textAlign};
  padding: ${style.cell.padding}px;
  border-bottom: ${style.cellBorder};
  border-right: ${style.cellBorder};
}`)

  parts.push(`#preview-content td:last-child { border-right: none; }`)
  parts.push(`#preview-content tr:last-child td { border-bottom: none; }`)

  if (style.stripedRows) {
    parts.push(`#preview-content tbody tr:nth-child(even) td {
  background-color: ${style.stripeColor};
}`)
  }

  if (style.hoverHighlight && !forPrint) {
    parts.push(`#preview-content tbody tr:hover td {
  background-color: ${style.hoverColor};
}`)
  }

  if (forPrint) {
    parts.push(`#preview-content tr { page-break-inside: avoid; }`)
    parts.push(`#preview-content thead { display: table-header-group; }`)
  }
...
#preview-content a:hover {
  ...
  // Table CSS
  const tableStyle = styles.table ?? DEFAULT_TABLE_STYLE
  rules.push(generateTableCSS(tableStyle))
...
export function generatePrintCSS(
  ...
  const tableStyle = styles.table ?? DEFAULT_TABLE_STYLE
  const printTableStyle: TableStyle = {
    ...tableStyle,
    width: '100%',
    hoverHighlight: false,
  }
  const printTableCSS = generateTableCSS(printTableStyle, true)

  // Generate base CSS but override table CSS with print version
  const baseCSS = generateCSS(styles, 'light', customRules)
...
// ---- Syntax color overrides for highlight.js (markdown preview code blocks) ----
//
// highlight.js uses semantic class names. We map our SyntaxColors tokens to them.
// These rules are injected as a <style> after the CDN stylesheet so they take priority.
// Only applied in light mode — dark mode uses the CDN github-dark theme as-is.

export function generateSyntaxColorsCSS(colors: SyntaxColors): string {
  return `
/* Syntax color overrides — light mode only */
.hljs-comment, .hljs-quote, .hljs-meta, .hljs-doctag { color: ${colors.comment} !important; }
.hljs-string, .hljs-regexp, .hljs-template-string, .hljs-template-tag, .hljs-addition { color: ${colors.string} !important; }
.hljs-keyword, .hljs-selector-tag, .hljs-built_in, .hljs-literal, .hljs-type { color: ${colors.keyword} !important; }
.hljs-number { color: ${colors.number} !important; }
.hljs-variable, .hljs-params { color: ${colors.variable} !important; }
.hljs-title, .hljs-title.class_, .hljs-title.function_ { color: ${colors.function} !important; }
.hljs-class .hljs-title, .hljs-type { color: ${colors.type} !important; }
.hljs-operator, .hljs-punctuation { color: ${colors.punctuation} !important; }
.hljs-property, .hljs-attr, .hljs-attribute { color: ${colors.property} !important; }
`
}
